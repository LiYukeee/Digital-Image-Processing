function result=metrics_(Y1, Y2)
...
% fprintf('\t MAE %f \t GMSD %f \t NIQE %f \n', MAE, GMSD, NIQE);
result = [MAE, GMSD, NIQE];
end
...
gt_path = "./gt/";
origin_path = "./images/";
% method_list = ["origin", "gamma", "ldr", "mf"];
method_list = ["gamma", "ldr", "mf"];
output_path = "./res/";
image_names = ["1.png", "497.png", "748.png", "776.png", "780.png"];

for method = method_list
    res = [0, 0, 0];
    for image = image_names
        Y1 = gt_path + image;
        Y2 = output_path + method + "-" + image;
        % fprintf(method + "-" + image);
        res = res +  metrics_(Y1, Y2);
    end
    avg_res = res / length(image_names);
    fprintf('%s \t MAE: %f \t GMSD: %f \t NIQE: %f\n', method, avg_res(1), avg_res(2), avg_res(3));
end
